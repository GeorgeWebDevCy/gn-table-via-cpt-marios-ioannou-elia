...
class Gn_Table_Via_Cpt_Marios_Ioannou_Elia_Public {
				...
				<table class="gn-works-table">
					<thead>
						<tr>
							...
							<th class="gn-col-year gn-sortable" data-sort="year" data-order="desc">Year <span class="gn-sort-icon">▼</span></th>
	...
	private function render_pagination( $page, $search, $orderby, $order, $per_page ) {
		...
		for ( $i = 1; $i <= $total_pages; $i++ ) {
			$link_classes = ( $i === $page )
				? 'gn-page-link page-link button -unlink -pagination -default'
				: 'gn-page-link page-link button -unlink -pagination -default -flat';
			$current_page = ( $i === $page ) ? ' aria-current="page"' : '';

			echo '<li class="page-item">';
			echo '<a href="#" class="' . esc_attr( $link_classes ) . '" data-page="' . intval( $i ) . '" aria-label="Page"' . $current_page . '>' . intval( $i ) . '</a>';
			echo '</li>';
		}
